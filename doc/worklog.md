...
## 複数モニターのシミュレーション（2026-09）

自前のモニターシミュレーションは最初から画面番号でキャンバスを引く作りになっていて、
`SimStubServer` も `screens` を Map で持っていた。つまりワイヤと描画は最初から N 画面
対応で、足りなかったのは**画面を増やす手段**だけだった。ゲーム内ではモニターの数は
ブロックの繋ぎ方で決まるものなので、マイコンのスクリプトを書き換えないと画面が増やせない
という状態は不便だった。

**Lua には手を入れずに済んだ** — LifeBoatAPI の `Simulator.lua` を読み直したところ、
描画ループは `for _, screenData in pairs(self._screens)` で**電源が入っている全画面**に
対して `onDraw` を呼んでおり、しかも `SCREENSIZE` ハンドラが

```lua
self._screens[screenNumber] = self._screens[screenNumber] or SimulatorScreen:new(screenNumber)
```

と、無い画面をその場で作る。`SimulatorScreen:new` は `poweredOn = true` で始まるので、
`SCREENSIZE|2|64|64` を送るだけで2枚目のモニターが生える。`SCREENPOWER` で消せる。
`_simulator.lua` へのパッチも、ユーザースクリプトの変更も要らない。

**踏みそうだった順序の罠** — `_beginSimulation` は接続を張った直後に既定の
`setScreen(1, "3x3")` を送る。パネルのレイアウトを接続時に押し込むと、この既定値が
後から上書きしてしまい「画面1のサイズだけ戻る」という分かりにくい壊れ方をする。
最初の `SCREENCONFIG` を受け取った時点で押し込むようにした（`pushWantedScreens`）。
接続ごとに1回だけなのも意図的で、`onLBSimulatorTick` から毎ティック `setScreen` を
呼ぶスクリプトとパネルが延々と殴り合うのを防いでいる。スクリプトが触った画面番号は
スクリプトの勝ち、というのが正しい優先順位。

**取り外しは電源OFF** — Lua 側に `_screens` からエントリを消す手段が無いので、
`SCREENPOWER|n|0` を送ってパネルから落とす。再度追加すれば戻る。画面1だけは外せない
ようにした（LifeBoatAPI の既定画面で、サイズとタッチがコンポジット入力へ流れる唯一の
画面）。

**倍率は全画面共通にした** — 従来の `scaleFor(w)` は画面ごとに「列幅に収まる最大の
整数倍率」を選んでいた。1画面なら問題ないが、複数画面だと 1x1 が 8倍・3x3 が 2倍に
なって、**小さいモニターの方が大きく表示される**。相対的な大きさが分からなくなるのは
複数モニター表示として致命的なので、`fitScale()`（`media/monitorConfig.js`）が
「全部を1行に並べたときに収まる最大倍率」を1つ返す形にした。1倍でも1行に入らない場合は
折り返して、いちばん幅の広い画面に合わせる。高さは従来どおり見ておらず、はみ出せば
スクロールする。

**タッチの合流状態をキャンバスごとにした** — `movePending` / `queuedMove` がモジュール
変数だったため、画面Aの pointerup が画面Bの保留中の move を消せた。`attachTouch` の
クロージャに移した。

**ホスト側のサイズ検証は緩めにしてある** — ドロップダウンが出すのは実在するサイズだけ
だが、スクリプトが `setScreen(1, "3x1")` のような組み合わせを宣言することはある。
パネルはそれをドロップダウンに追加して表示するので、ホストが表の外を弾くと
「その画面の Portrait チェックボックスだけ効かない」という死んだ操作ができてしまう。
`sanitizeScreenRequest` は `NxM`（各辺1〜9ブロック）なら通す。
